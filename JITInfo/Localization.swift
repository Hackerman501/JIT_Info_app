...
private let enStrings: [String: String] = [
    ...
    "expl.availableMemory": "Amount of memory the process may safely allocate \u{2013} depends on the kernel entitlements.",
    "expl.ramShare": "Share of physical RAM the app may use. Above 60 % points to a raised limit.",
    "expl.heuristic": "Heuristic: If os_proc_available_memory > 60 % of RAM, a raised limit is likely active.",
...
private let deStrings: [String: String] = [
    ...
    "expl.availableMemory": "Speichermenge, die der Prozess sicher allokieren darf \u{2013} h\u{00E4}ngt von den Kernel-Entitlements ab.",
    "expl.ramShare": "Anteil des physikalischen RAMs, den die App nutzen darf. \u{00DC}ber 60 % deutet auf ein erh\u{00F6}htes Limit hin.",
    "expl.heuristic": "Heuristik: Ist os_proc_available_memory > 60 % des RAMs, wirkt vermutlich ein erh\u{00F6}htes Limit.",
